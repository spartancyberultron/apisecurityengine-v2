...
const SOAPGraphQLScans = () => {

  //const [toast, addToast] = useState(0)
  const navigate = useNavigate()

  const [soapOrGraphQLScans, setSoapOrGraphQLScans] = useState([])
  ...
  const deleteScan = async (id) => {
    ...
    try {
      ...
      const response = await axios.post('api/v1/soapOrGraphQLScans/deleteSOAPOrGraphQLScan', requestBody, {
        headers: {
  ...
  const goToViewReport = async (scanId) => {
    ...
    navigate('/view-soap-graphql-scan-report?scanId=' + scanId);
  };
  ...
  for (var i = 0; i < soapOrGraphQLScans.length; i++) {
    ...
    dataItem.push((new Date(soapOrGraphQLScans[i].createdAt)).toLocaleDateString('en-US') + 
    ' - ' + (new Date(soapOrGraphQLScans[i].createdAt)).toLocaleTimeString('en-US'));

    if (soapOrGraphQLScans[i].scanCompletedAt) {
      dataItem.push((new Date(soapOrGraphQLScans[i].scanCompletedAt)).toLocaleDateString('en-US')
        + ' - ' + (new Date(soapOrGraphQLScans[i].scanCompletedAt)).toLocaleTimeString('en-US'));
    } else {
      dataItem.push('---');
    }
   ...
   dataItem.push(soapOrGraphQLScans[i].status?(soapOrGraphQLScans[i].status).toUpperCase():'COMPLETED');    
    ...
    dataItem.push(soapOrGraphQLScans[i]._id); // for delete

    tableData.push(dataItem);
  }

  console.log("soapOrGraphQLScans",soapOrGraphQLScans)



  const goToStartSOAPOrGraphQLScan = (e) => {

    e.preventDefault();
    navigate('/start-soap-graphql-scan')
  }
      ...
      <div style={{ width: '100%' }}>
        <div>
          <div style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <span className="pageHeader">SOAP/GraphQL Scans</span>

            <CButton
              className="primaryButton"
              
              onClick={goToStartSOAPOrGraphQLScan}
              color="primary"
            >
              <AiOutlineSecurityScan size={20} color='white'/>

              <span className="primaryButtonText" style={{marginLeft:10}}>Start a SOAP/GraphQL Scan</span>
            </CButton>
          </div>
...
export default SOAPGraphQLScans
