...
const SBOMScans = () => {
  ...
  const handlePageClick = (event) => {
    ...
    fetchSBOMScans(true, requestedPage);
  ...
  const deleteScan = async (id) => {
    ...
    try {
      ...
      const response = await axios.post('api/v1/sbomScans/deleteSBOMScan', requestBody, {
        headers: {
  ...
  const fetchSBOMScans = async (isFirstTime,  page, rowsPerPage) => {
    ...
    const response = await axios.get(`/api/v1/sbomScans/getAllSBOMScans/${page}/${rowsPerPage}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  
    setSBOMScans(response.data.sbomScans);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      fetchSBOMScans(false, page, rowsPerPage);

    }, 30000);
    

    // Make the initial call immediately
    fetchSBOMScans(true, 0, rowsPerPage);
  ...
  const goToViewReport = async (scanId) => {
    ...
    navigate('/view-sbom-scan-report?scanId=' + scanId);
  };
  ...
  const options = {
    ...
    onTableChange: (action, tableState) => {
      if (action === 'changePage' || action === 'changeRowsPerPage') {
        ...
        fetchSBOMScans(true, page, rowsPerPage);
      }
    }
  };
  ...
  for (var i = 0; i < sbomScans.length; i++) {
    ...
    if (sbomScans[i].scanCompletedAt) {
      dataItem.push((new Date(sbomScans[i].scanCompletedAt)).toLocaleDateString('en-US')
        + ' - ' + (new Date(sbomScans[i].scanCompletedAt)).toLocaleTimeString('en-US'));
    } else {
      dataItem.push('---');
    }
    ...
    dataItem.push(sbomScans[i]); // for view report link
    dataItem.push(sbomScans[i]._id); // for delete

    tableData.push(dataItem);
  }

  const goToStartSBOMScan = (e) => {

    e.preventDefault();
    navigate('/start-sbom-scan')
  }
      ...
      <div style={{ width: '100%' }}>
        <div>
          <div style={{ marginBottom: '2rem', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <span className="pageHeader">SBOM Scans</span>

            <CButton
              className="primaryButton"              
              onClick={goToStartSBOMScan}
              color="primary"
            >
              <AiOutlineSecurityScan size={20} color='white'/>

              <span className="primaryButtonText" style={{marginLeft:10}}>Start a SBOM Scan</span>
            </CButton>
          </div>
